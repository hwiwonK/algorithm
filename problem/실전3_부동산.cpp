...
int main() {
	...
	for (int tc = 1; tc <= T; tc++) {
		...
		//1열, 1행 채우기
		for (int j = 1; j <= C; j++) {
			dpArrUp[1][j][0] = dpArr[1][j][0] = gain[1][j];
			dpArrUp[1][j][1] = dpArr[1][j][1] = 1;
			dpArrUp[1][j][2] = dpArr[1][j][2] = j;

			if (dpArrLeft[1][j - 1][0] + gain[1][j] > gain[1][j]) {
				dpArrLeft[1][j][0] = dpArrLeft[1][j - 1][0] + gain[1][j];
				dpArrLeft[1][j][1] = dpArrLeft[1][j - 1][1];
				dpArrLeft[1][j][2] = dpArrLeft[1][j - 1][2];
			}
			else {
		...
		for (int i = 2; i <= R; i++) {
			dpArrLeft[i][1][0] = dpArr[i][1][0] = gain[i][1];
			dpArrLeft[i][1][1] = dpArr[i][1][1] = i;
			dpArrLeft[i][1][2] = dpArr[i][1][2] = 1;
			
			if (dpArrUp[i - 1][1][0] + gain[i][1] > gain[i][1]) {
				dpArrUp[i][1][0] = dpArrUp[i - 1][1][0] + gain[i][1];
				dpArrUp[i][1][1] = dpArrUp[i - 1][1][1];
				dpArrUp[i][1][2] = dpArrUp[i - 1][1][2];
			}
			else {
		...
		for (int i = 2; i <= R; i++) {
			for (int j = 2; j <= C; j++) {
				...
				//대각선으로 모든 경우 확인해야 함
				int r = dpArr[i-1][j-1][1], c = dpArr[i-1][j-1][2];
				...
				if (dpArr[i - 1][j - 1][0] + sumTmp > gain[i][j]) {
					dpArr[i][j][0] = dpArr[i - 1][j - 1][0] + sumTmp;
					dpArr[i][j][1] = dpArr[i - 1][j - 1][1];
					dpArr[i][j][2] = dpArr[i - 1][j - 1][2];
				}
				else {
